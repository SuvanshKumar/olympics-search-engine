...
solr_vector = pysolr.Solr('http://192.168.0.2:8983/solr/project_index_vector/', always_commit=True, timeout=10)
solr_page_rank = pysolr.Solr('http://192.168.0.2:8983/solr/project_index_pagerank/', always_commit=True, timeout=10)
...
query_list = ["medal", "winner", "games", "athlete", "summer", "winter", "mens"]
    ...
    if 'query' in request.args and 'type' in request.args:
        ...
        if "clustering" in type:
            ...
            if 'association' in type:
                ...
                n = 1
                random_terms = np.random.choice(query_list, n, replace = False)
                random_terms = np.random.choice(query_list, n)
                solr_results = get_results_from_solr_page_rank(query)
